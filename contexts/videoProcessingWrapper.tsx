...
const VideoProcessingWrapper: React.FunctionComponent = ({ children }) => {
  ...
  const handlerSocketUpdated = useCallback((data: any) => {
    ...
    if (decoded.postUuid && decoded.fractionCompleted === 100 && decoded.status === newnewapi.VideoProcessingProgress.Status.SUCCEEDED) {
    ...
    if (decoded.postUuid && decoded.status === newnewapi.VideoProcessingProgress.Status.FAILED) {
