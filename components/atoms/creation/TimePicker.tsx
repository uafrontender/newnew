...
export const TimePicker: React.FC<ITimePicker> = (props) => {
  ...
  const handleFormatChange = useCallback((value: any) => {
    onChange('hours-format', value);
  }, [onChange]);

  const renderHourItem = useCallback((item: any) => {
  ...
  const renderMinuteItem = useCallback((item: any) => {
  ...
  const renderFormatItem = useCallback((item: any) => {
    ...
    <SWrapper ref={wrapperRef}>
      ...
      >
        <SListHolder direction={direction}>
          ...
          <SScrollList noPadding>
            {FORMAT.map(renderFormatItem)}
          </SScrollList>
        </SListHolder>
      </AnimatedPresence>
    </SWrapper>
...
const SItem = styled.div<ISItem>`
  ...
  :hover {
    background: ${(props) => (props.selected ? props.theme.colorsThemed.accent.blue : props.theme.colorsThemed.background.quaternary)};
  }
`;
