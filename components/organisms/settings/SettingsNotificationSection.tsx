...
const SettingsNotificationsSection: React.FunctionComponent<TSettingsNotificationsSection> = ({
  ...
  return (
    ...
    >
      {configs && configs.map((subsection, idx) => (
        <SSubsection
        ...
        >
          ...
          <SItemsContainer>
            {subsection.configs && subsection.configs.map((config, i, arr) => (
              <SItem
              ...
              >
                ...
                <Toggle
                  ...
                  onChange={() => handleUpdateItem(subsection.parameter, config.parameter)}
                />
              </SItem>
            ))}
          </SItemsContainer>
...
const SItem = styled.div<{
  ...
  border-bottom: ${({ isLast, isLastAll, theme }) => (isLast && !isLastAll ? `1px solid ${theme.colorsThemed.background.outlines1}` : 'unset')};
